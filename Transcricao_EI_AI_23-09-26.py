...
import tiktoken  #pip install openai-whisper
import FreeSimpleGUI as sg # Alterando para a versão 100% Gratis
# c:\users\regenilson-dev\appdata\local\pip\cache\wheels\
# Corrigindo erro do Windows[206] - Caminho do arquivo de cache do pip muito longo PowerShell("New-ItemProperty -Path "HKLM:\System\CurrentControlSet\Control\FileSystem" -Name "LongPathsEnabled" -Value 1 -PropertyType DWORD -Force")
import datetime
...
# Para instalar as dependências necessárias do projeto, execute o seguinte comando no terminal:
    ...
    Gerar o arquivo: pip freeze lista as bibliotecas instaladas; pip freeze > requirements.txt salva no arquivo
...
def create_singleton():
    '''
    O Singleton garante que uma classe tenha apenas uma instância em execução.
    '''
    app_name   = "Transcricao_BotRegis_I_A"
...
def verificar_checksum_modelo(modelo_path):
    try:
        ...
        tamanhos_esperados = {
            'tiny.pt':   39_000_000,     
            'base.pt':   145_000_000,    
            'small.pt':  488_000_000,    
            'medium.pt': 1_540_000_000,  
            'large':     3_090_000_000,  
        }
        ...
        try:
            ...
            if tentativa > 0:
                ...
                if modelo_escolhido == 'large':
                    ...
                    limpar_cache_completo()  # Corrigido: Adicionado os parênteses () para executar a função
            ...
            if "SHA256 checksum" in error_msg:
                ...
                if modelo_escolhido == 'large' and tentativa == max_tentativas - 1:
                    print("🔄 Última tentativa falhada para 'large'. Tentando alternatives...")
                    try:
...
def salvar_transcricao_automatica(texto, modelo_usado):
    '''
    Função que salva automaticamente a transcrição em um arquivo TXT estruturado.
    '''
    try:
        if not texto or not texto.strip():
            return None
        
        # Cria a pasta 'transcricoes_automaticas_botregis' se não existir
        pasta_destino = "transcricoes_automaticas_botregis"
        if not os.path.exists(pasta_destino):
            os.makedirs(pasta_destino)
            
        # CORREÇÃO: Mudamos '/' para '-' para que o Windows aceite o nome do arquivo
        timestamp = datetime.now().strftime('%d-%m-%Y_%H%M%S')
        nome_arquivo = f"transcricao_{timestamp}_{modelo_usado}.txt"
        caminho_completo = os.path.join(pasta_destino, nome_arquivo)
        
        # Cria o cabeçalho organizacional do seu Bot (mantendo barras apenas dentro do texto)
        cabecalho = (
            f"="*60 + "\n"
            f"🤖 BOT-REGIS - RELATÓRIO DE TRANSCRIÇÃO AUTOMÁTICA\n"
            f"📅 Data/Hora: {datetime.now().strftime('%d/%m/%Y %H:%M:%S')}\n"
            f"🧠 Modelo Inteligência Artificial: {modelo_usado.upper()}\n"
            f"="*60 + "\n\n"
        )
        ...
        # Adicionei um print aqui também para você ver o erro no sg.Output se algo mais falhar
        print(f"❌ Erro crítico ao salvar TXT: {e}")
        log.critical(f"Erro ao salvar arquivo TXT automaticamente: {e}")
        return None
...
def transcrever_audio(audio_file, modelo_escolhido, window):
    ...
    try:
        ...
        print("")
        print(f"🎯 Iniciando transcrição ás {data_ano} com modelo {modelo_escolhido}...\n")
        ...
        if modelo_usado != modelo_escolhido:
            print(f"⚠️ Usando modelo '{modelo_usado}' invez de '{modelo_escolhido}'")
        ...
        if result and result.get("text"):
            ...
            print(f"📝 Transcrição concluída com sucesso ás {datetime.now().strftime('%d-%m-%Y %H:%M:%S')} com modelo {modelo_usado}!\\n")
            print(f'Resultado da transcrição ==> áudio:MP3 -> Modelo: {modelo_usado}\n')
            
            for sentence in texto_final.split("."):
                if sentence.strip():
                    print(sentence.strip())
                    
            data_ano = datetime.now().strftime('%d-%m-%Y %H:%M:%S')
            print(f'\n🎙️ Áudio transcrito com sucesso🎙️!')
            print(f'🔏 Terminamos a transcrição do áudio no DIA: {data_ano}.')
            
            # --- SALVAMENTO AUTOMÁTICO ---
            caminho_txt = salvar_transcricao_automatica(texto_final, modelo_usado)
            if caminho_txt:
                print(f"\n💾 [SALVAMENTO AUTOMÁTICO]: O arquivo foi gerado com sucesso em: '{caminho_txt}'")
                log.info(f"Arquivo salvo automaticamente em {caminho_txt}")
...
def main():
    ...
    while True:      
        ...
        if eventos == '-TRANSCRICAO_COMPLETA-':
            ...
            sg.popup_quick_message("Transcrição concluída e salva automaticamente!", background_color="green", text_color="white")
            continue
